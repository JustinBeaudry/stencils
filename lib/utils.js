...
async function checkIfExists (fileName, altPath=null) {
  return util.promisify(fs.open)(getPath(fileName, altPath), 'r')
    .catch(err => {
      // if no such file or directory, or is a directory
      const isNotExistent = err && err.code === 'ENOENT' || err.code === 'EISDIR' || false;
 ...
 */
async function getDirectory(directory, altPath=null) {
  let exists;
  try {
    exists = await checkIfExists(directory, altPath);
  } catch(e) {
    return Promise.reject(e);
  }
  if (!exists) return Promise.reject(new Error('directory not found'));
  return util.promisify(fs.readdir)(getPath(directory, altPath));
}
