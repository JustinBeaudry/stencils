...
async function checkIfExists (fileName) {
  return new Promise((resolve, reject) => {
    fs.open(getPath(fileName), 'r', err => {
      ...
      if (err && !isNotExistent) reject(err);
...
async function getFiles(dir) {
  ...
  return util.promisify(fs.readdir)(getPath(dir));
}
...
async function getFile(file) {
  ...
  if (!exists) return new Error('file not found');
  return util.promisify(fs.readFile)(getPath(file));
}
...
async function getTemplates(templatesDir, templateExt) {
  ...
  if (!files) return new Error('no templates found');
