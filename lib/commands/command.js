...
module.exports = class Command {
  ...
  async getEngine(engineName) {
    ...
    if (!engine) throw new Error(`unsupported engine requested:  ${engineName}`);
    return engine;
  }
  ...
  async getTemplateNames() {
    ...
    if (!files) throw new Error('template files not found');
  ...
  async getMeta(templateName) {
    ...
    if (!file) throw new Error('meta file not found');
  ...
  async getConfig() {
    ...
    try {
      ...
      if (this.internals.exitOnConfigMissing) {
        throw new Error(`not a stencils project. ${this.chalk.bold.cyan('stpl init')} to initialize project for use with stencils.`);
      }
    }
    try {
      ...
      if (this.internals.throwOnConfigParseError) {
        throw new Error(`error parsing ${this.configPath} file`);
      }
    }
    if (rootConfig) {
      try {
