...
module.exports = class Command {
   ...
   * @param {String} (altPath) - used when finding directory templates
   * @returns {Promise.<Array<String>|Error>}
   */
  async getTemplateNames(altPath) {
    let files;
    let location = altPath ? path.join(this.templatesDir, altPath) : this.templatesDir;
    try {
      files = await utils.getFiles(location);
    } catch(e) {
  ...
  async getTemplate(templateName) {
    ...
    try {
      ...
      // if file is a directory, get the directory instead
      if (e && e.code === 'EISDIR') return this.getTemplateList(pathName);
      return Promise.reject(e);
    }
   ...
   * @returns {Promise.<Array<String>|Error>}
   */
  async getTemplateList(pathName) {
    let templateNames;
    let templateList;
    try {
      templateNames = await utils.getDirectory(pathName);
    } catch (e) {
      return Promise.reject(e);
    }
    try {
      templateList = await this.getTemplateFiles(pathName, templateNames);
    } catch(e) {
      return Promise.reject(e);
    }
    return templateList;
   ...
   * @returns {Promise.<Array<String>|Error>}
   */
  async getTemplateFiles(dir, templateNames) {
    let withoutMetaFiles = templateNames.filter(templateName => !/meta/.test(templateName));
    return new Promise((resolve, reject) => {
      async.map(withoutMetaFiles, async templateName => {
        const template = await utils.getFile(path.join(dir, templateName));
        let meta = await utils.getFile(path.join(dir, templateName + '.meta'));
        meta = JSON.parse(meta);
        if (!template) return new Error('templates not found');
        return {
          template: template,
          meta: meta
        };
